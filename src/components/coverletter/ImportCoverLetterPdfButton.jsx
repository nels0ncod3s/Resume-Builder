...
export default function ImportCoverLetterPdfButton({ onImport }) {
  ...
  async function handleChange(e) {
    ...
    try {
      ...
      onImport(parseCoverLetterText(text));
      setStatus("done");
    } catch {
      setStatus("error");
  ...
  return (
    <div data-tour="import-cover-letter-pdf" className="mb-6 rounded-lg border border-dashed border-line bg-canvas p-4">
      <p className="text-sm font-semibold text-ink">Already have a cover letter?</p>
      <p className="mt-0.5 text-xs leading-relaxed text-ink-soft">
        Upload a PDF to auto-fill the fields below. This replaces the current form, and parsing
      ...
      {status === "done" && (
        <p className="mt-2 text-xs font-medium text-green-600">Imported — review each field below.</p>
      )}
